...
describe Robot do
  ...
  describe "#heal!" do
    it "raises exception if health is <= 0" do
      @robot.wound(150)
      expect{ @robot.heal!(20) }.to raise_error(RobotIsDead)
    end
  end
  ...
  describe "#attack!" do
    it "raises exception if the attacked unit is not a robot" do
      attacked_unit = Item.new('Test item', 20)
      expect { @robot.attack!(attacked_unit) }.to raise_error(CannotAttackEnemy)
    end
  end
